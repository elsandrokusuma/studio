...
import { AlertCircle, ArrowDownLeft, ArrowUpRight, Package, ClipboardCheck, PackageX, DollarSign, Ban, Clock, BarChart4, Plus, TrendingUp, TrendingDown, FileText, Zap, Check, ChevronsUpDown, Sun } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const [stockStatusItems, setStockStatusItems] = React.useState<InventoryItem[]>([]);
  ...
  const handleShowLowStock = () => {
    ...
    setStockStatusItems(inventoryItems.filter(item => item.quantity <= 5 && item.quantity > 0));
  ...
  const stockValue = inventoryItems.reduce((acc, item) => acc + item.quantity * item.price, 0);
  ...
  const selectedItemForStockOut = inventoryItems.find(i => i.id === selectedItemId);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card onClick={() => router.push('/inventory')} className="cursor-pointer hover:shadow-lg transition-shadow bg-blue-500 text-white">
        ...
        <Card onClick={handleShowLowStock} className="cursor-pointer hover:shadow-lg transition-shadow bg-orange-400 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card onClick={handleShowOutOfStock} className="cursor-pointer hover:shadow-lg transition-shadow bg-red-500 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card onClick={() => router.push('/approval')} className="cursor-pointer hover:shadow-lg transition-shadow bg-purple-500 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="grid gap-3">
              <Button onClick={() => setAddOpen(true)} className="w-full justify-start h-auto p-4 bg-gradient-to-r from-blue-400 to-cyan-400 text-white hover:from-blue-500 hover:to-cyan-500">
                <div className="bg-white/20 p-2 rounded-lg mr-4">
                    <Plus className="h-5 w-5" />
                </div>
                <div>
                    <p className="font-semibold text-base">Add Item</p>
                    <p className="font-normal text-sm text-left">New inventory item</p>
                </div>
              </Button>
              <Button onClick={() => setStockInOpen(true)} className="w-full justify-start h-auto p-4 bg-gradient-to-r from-green-400 to-emerald-400 text-white hover:from-green-500 hover:to-emerald-500">
                <div className="bg-white/20 p-2 rounded-lg mr-4">
              ...
              <Button onClick={() => setStockOutOpen(true)} className="w-full justify-start h-auto p-4 bg-gradient-to-r from-red-400 to-pink-400 text-white hover:from-red-500 hover:to-pink-500">
                <div className="bg-white/20 p-2 rounded-lg mr-4">
              ...
              <Button onClick={() => setCreatePoOpen(true)} className="w-full justify-start h-auto p-4 bg-gradient-to-r from-purple-400 to-indigo-400 text-white hover:from-purple-500 hover:to-indigo-500">
                <div className="bg-white/20 p-2 rounded-lg mr-4">
                 ...
                 <span className="col-span-2">
                  <Badge
                    variant={
                     ...
                     className={
                        selectedTransaction.type === 'in' || selectedTransaction.type === 'add' ? 'bg-green-100 text-green-800' :
                        selectedTransaction.type === 'edit' ? 'bg-gray-100 text-gray-800' :
                        'bg-red-100 text-red-800'
                      }
                  >
      ...
      <Dialog open={isStockStatusOpen} onOpenChange={setStockStatusOpen}>
        <DialogContent className="h-[60vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              A list of all items that are currently {stockStatusTitle.toLowerCase().replace(' items', '')}.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleAddItem} className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
             ...
             <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="price" name="price" type="number" className="col-span-3" required min="0" />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="photoUrl" name="photoUrl" placeholder="Optional: https://..." className="col-span-3" />
            </div>
            <DialogFooter>
      ...
      <Dialog open={isStockOutOpen} onOpenChange={(isOpen) => { if(!isOpen) { setSelectedItemId(null); setSelectedItemName("Select an item..."); } setStockOutOpen(isOpen); }}>
        <DialogContent>
          ...
          <form onSubmit={handleStockUpdate("out")} className="grid gap-4 py-4">
            ...
            <DialogFooter>
              <Button type="submit" disabled={!selectedItemId}>Remove Stock</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isCreatePoOpen} onOpenChange={(isOpen) => { if(!isOpen) { setSelectedItemId(null); setSelectedItemName("Select an item..."); } setCreatePoOpen(isOpen); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the details for the new pre-order. This will create a single item PO.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreatePreOrder} className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
             ...
             <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="price" className="text-right">Price</Label>
              <Input id="price" name="price" type="number" min="1" className="col-span-3" required />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="unit" className="text-right">Unit</Label>
